...
const coordenadasZ = [];
let largoTotal = 0;
let cilCount = 1;  // Contador de fuerzas agregadas
...
function addCilInput() {
    ...
    div.className = 'entrada-cil';
    div.innerHTML = `
        <label>Cilindro ${cilCount}:</label>
        <div class="input-group">
            <label for="diameter${cilCount}">Diametro del cilindro ${cilCount} : </label>
            <input type="number" id="diameter${cilCount}" value="0" step="0.1">
            <span id="diameterUnit" class="unit-label">mm</span>
        </div>
        <div class="input-group">
            <label for="length${cilCount}">Largo del cilindro ${cilCount} : </label>
...
// Función para guardar los valores de todos los inputs generados
function saveCil() {
    for (let i = 1; i <= cilCount; i++) {
        const diametro = document.getElementById('diameter' + i).value;
        const largo = document.getElementById('length' + i).value;
        guardarCilindro(diametro, largo);
        createCylinderEsc2D()
    }
    
}
// Guarda las dimensiones del cilindro
function guardarCilindro(diametro, largo) {
...
function actualizarListaCil() {
    ...
    diametros.forEach((diam, index) => {
        let li = document.createElement("li");
        li.textContent = `Cilindro ${index + 1}: Diametro = ${diam}, Largo = ${largos[index]} `;
